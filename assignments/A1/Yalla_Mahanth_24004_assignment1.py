...
Original file is located at
    https://colab.research.google.com/drive/1UiuLSwHPrUxlsoQ_7HCiMd7Nc_b6Pys2
...
STUDENT_SAP_NAME  = "Yalla Mahanth"
...
class NaiveBayesClassifier:
    ...
    def calculate_word_probabilities(self, X, y, vocabulary, alpha=1.0):
        ...
        total_words_in_class = np.array([np.sum(X[y_ == cls]) for cls in classes])
        den = total_words_in_class + alpha * vocab_size

        counts = np.array( [ np.sum(X[y_ == cls][:, np.arange(vocab_size)], axis=0) for cls in classes])
        num = counts + alpha
    ...
    def predict(self, X_text):
        ...
        for text in X_text:
            ...
            for cls in self.class_probs.keys():
                for word in row:
                    probs[cls] += np.log(self.word_probs[cls].get(word, self.word_probs[cls][self.UNK]))
...
class WordEmbeddingOps:
    ...
    def find_analogies(self, word1, word2, word3, similarity_func='cosine', num_results=5):
        ...
        try:
            word4_vec = self.word_vectors[word2] - self.word_vectors[word1] + self.word_vectors[word3]
        except KeyError:
            return []
        if similarity_func == 'cosine':
            ...
            scrs /= w4_norm
            vec_norms = np.linalg.norm(self.word_vectors.vectors,axis = 1).reshape(-1,1)
            scrs /= vec_norms
            scores = [ (word,score) for word,score in zip(self.word_vectors.index_to_key,scrs.reshape(-1).tolist()) if word not in ex_words]
        else:
            scrs = np.linalg.norm(self.word_vectors.vectors - word4_vec,axis = 1)
            scrs = 1 / (1 + scrs)
            scores = [ (word,score) for word,score in zip(self.word_vectors.index_to_key,scrs) if word not in ex_words]
    ...
    def find_similar_words(self, word, num_results=5, similarity_func='cosine'):
        ...
        if similarity_func == 'cosine':
            ...
            vec_norms = np.linalg.norm(self.word_vectors.vectors,axis = 1).reshape(-1,1)
            scrs /= vec_norms
            scores = [ (word_i,score) for word_i,score in zip(self.word_vectors.index_to_key,scrs.reshape(-1).tolist()) if word_i != word]
        else:
            scrs = np.linalg.norm(self.word_vectors.vectors - word_vec,axis = 1)
            scrs = 1 / (1 + scrs)
            scores = [ (word_i,score) for word_i,score in zip(self.word_vectors.index_to_key,scrs) if word_i != word]
        return sorted(scores,key=lambda x: x[1], reverse=True)[:num_results]
...
class BagOfWordsClassifier:
    ...
    def create_vocabulary(self, texts):
        ...
        for txt in texts:
            for word in self.preprocess_text(txt):
                self.freqeuncies[word] = self.freqeuncies.get(word, 0) + 1
        return {k: i for i, (k, v) in enumerate(self.freqeuncies.items()) if v >= self.min_freq}
        ...
        for txt in texts :
            ...
            for word in self.preprocess_text(txt):
                try :
                    ...
                    continue
            res.append(np.mean(np.array(vecs),axis = 0) if len(vecs) > 0 else np.zeros((vec_size)))
...
print('\n\n\n' +'*'*50 + '\nTime Analysis\n' + '*'*50)
print(f'Time taken to Run Section 1: {section_1 - start_time}')
print(f'Time taken to Run Section 2: {section_2 - section_1}')
print(f'Time taken to Run Section 3: {end_time - section_2}')
print(f'Time taken to Run entire code: {end_time - start_time}')